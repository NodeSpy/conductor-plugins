...
// program's results become the step's outputs. There is no ctx.store,
...
func describe() plugin.Decl {
	return plugin.Decl{
		...
		Desc: "jq code steps on gojq (pure Go, no cgo): the step inputs are the jq " +
			...
			"results at all is no outputs.",
		Capabilities: plugin.Capabilities{},
	}
}

func run(ctx context.Context, req plugin.RunRequest, host *plugin.Host) (plugin.RunResult, error) {
	fmt.Fprintf(os.Stderr, "conductor-jq: run instance=%s inputs=%d\n", req.Instance, len(req.Inputs))
	outputs, err := execJQ(ctx, req.Code, req.Inputs)
	if err != nil {
...
func execJQ(ctx context.Context, code string, inputs map[string]any) (map[string]any, error) {
	...
	iter := program.RunWithContext(ctx, input)
	var results []any
	for {
		v, ok := iter.Next()
		if !ok {
