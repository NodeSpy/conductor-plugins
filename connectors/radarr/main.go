// Command conductor-radarr is the Radarr connector as a standalone external
// conductor plugin (#59). It drives a self-hosted Radarr instance's REST API
// v3 as verbs (movie listing/lookup/add/delete, commands, queue, calendar,
// wanted/missing, quality profiles, root folders, health, and a generic
// `api` escape hatch), and as a SOURCE it receives Radarr's own Webhook
// notification POSTs (Grab, Download, MovieAdded, MovieDelete,
// MovieFileDelete, Health, etc. — whatever the operator wires up in Radarr's
...
//
// Radarr itself has no outbound webhook signing: its Connect > Webhook
...
// Connection:
//
//	base_url: "http://radarr:7878" # REQUIRED: base of the instance (no trailing /api/v3)
...
// scope `network:` on the connector instance to their own Radarr host (see
// docs/connectors/radarr.md).
//
...
type radarrPlugin struct{}

func (radarrPlugin) Describe() plugin.Decl {
	return plugin.Decl{
		Kind: plugin.KindConnector,
		Type: "radarr",
		Desc: "Radarr: movie management as verbs (movies, lookup, add/delete movie, command, queue, calendar, wanted_missing, quality_profiles, root_folders, health, api escape hatch) over REST API v3; Grab/Download/movie lifecycle events in via Radarr's own Webhook connection (source).",
		...
		Verbs:  radarrVerbs(),
		// Radarr is always self-hosted — there is no fixed hostname this
...
func radarrEvent() plugin.Event {
	return plugin.Event{
		Name: "event",
		Desc: "a Radarr webhook notification fired (Grab, Download, MovieAdded, MovieDelete, MovieFileDelete, Health, Test, etc. — whatever notifications you enable on the Webhook connection in Radarr)",
		Filters: plugin.Schema{
			"event_types": {Type: "list", Desc: "match payload.eventType against any of these (empty = any)"},
			"movies":      {Type: "list", Desc: "match the movie title against any of these (empty = any)"},
		},
		Context: plugin.Schema{
			"event_type":  {Type: "string", Desc: "e.g. Grab, Download, MovieAdded, MovieDelete, MovieFileDelete, Health, Test"},
			"movie_title": {Type: "string"},
			"year":        {Type: "integer"},
			"tmdb_id":     {Type: "integer"},
			"quality":     {Type: "string", Desc: "the release/movie file quality name, when present"},
			"payload":     {Type: "any", Desc: "the full posted JSON body, verbatim"},
		},
	}
}

func radarrVerbs() []plugin.Verb {
	...
	return []plugin.Verb{
		{
			Name:    "movies",
			Desc:    "list all movies known to Radarr",
			Outputs: itemsOut,
		},
		{
			Name: "movie_get",
			Desc: "a single movie by id",
		...
		{
			Name: "lookup",
			Desc: "search for a movie to add (TheMovieDB search)",
			Options: plugin.Schema{
				"term": {Type: "string", Required: true, Desc: "search term, e.g. a title or tmdb:12345"},
			},
			Outputs: itemsOut,
		},
		{
			Name: "add_movie", Desc: "add a movie to Radarr",
			Usage: "either pass tmdb_id/quality_profile_id/root_folder_path (built into the request), or pass the full `movie` map for direct passthrough",
			Options: plugin.Schema{
				"tmdb_id":              {Type: "integer", Desc: "TheMovieDB id (required unless `movie` is given)"},
				"quality_profile_id":   {Type: "integer", Desc: "required unless `movie` is given"},
				"root_folder_path":     {Type: "string", Desc: "required unless `movie` is given"},
				"monitored":            {Type: "boolean", Desc: "default true"},
				"minimum_availability": {Type: "string", Desc: "default released"},
				"search_for_movie":     {Type: "boolean", Desc: "addOptions.searchForMovie"},
				"movie":                {Type: "map", Desc: "full Radarr movie object, sent verbatim instead of the individual fields above"},
			},
			Outputs: resultOut,
		},
		{
			Name: "delete_movie", Desc: "remove a movie from Radarr",
			Options: plugin.Schema{
				"id":                   {Type: "integer", Required: true},
				"delete_files":         {Type: "boolean", Desc: "also delete the movie's files on disk"},
				"add_import_exclusion": {Type: "boolean", Desc: "add the movie to the import list exclusion list"},
			},
			Outputs: resultOut,
		},
		{
			Name: "command", Desc: "run a Radarr command",
			Usage: "e.g. MoviesSearch, MovieSearch, RefreshMovie, RescanMovie",
			Options: plugin.Schema{
				"name":      {Type: "string", Required: true, Desc: "command name, e.g. MoviesSearch, MovieSearch, RefreshMovie, RescanMovie"},
				"movie_ids": {Type: "list", Desc: "list of movie ids"},
		...
		{
			Name: "calendar", Desc: "movies releasing in a date range",
			Options: plugin.Schema{
		...
		{
			Name:    "wanted_missing",
			Desc:    "movies Radarr considers missing",
			Outputs: resultOut,
		},
		{
		...
		{
			Name: "api", Desc: "call any Radarr API v3 endpoint not covered by a first-class verb",
...
func buildRequest(verb string, o map[string]any) (reqBuild, error) {
	switch verb {
	case "movies":
		return reqBuild{method: http.MethodGet, path: "/movie", query: url.Values{}, asList: true}, nil
	...
	case "movie_get":
		...
		return reqBuild{method: http.MethodGet, path: "/movie/" + strconv.FormatInt(id, 10), query: url.Values{}}, nil
	...
	case "lookup":
		...
		return reqBuild{method: http.MethodGet, path: "/movie/lookup", query: url.Values{"term": {term}}, asList: true}, nil

	case "add_movie":
		return buildAddMovie(o)
	...
	case "delete_movie":
		...
		return reqBuild{method: http.MethodDelete, path: "/movie/" + strconv.FormatInt(id, 10), query: q}, nil
...
// buildAddMovie builds POST /movie. When options.movie is a map it is sent
// verbatim (full passthrough); otherwise the request body is built from the
// individual fields.
func buildAddMovie(o map[string]any) (reqBuild, error) {
	if movie, ok := o["movie"].(map[string]any); ok {
		return reqBuild{method: http.MethodPost, path: "/movie", query: url.Values{}, body: movie}, nil
	}
	tmdbID, err := requiredInt(o, "tmdb_id")
	if err != nil {
		return reqBuild{}, err
	}
	...
	body := map[string]any{
		...
		"monitored":           monitored,
		"minimumAvailability": strOr(o["minimum_availability"], "released"),
		"addOptions": map[string]any{
			"searchForMovie": boolv(o["search_for_movie"]),
		},
	}
	return reqBuild{method: http.MethodPost, path: "/movie", query: url.Values{}, body: body}, nil
}

// buildCommand builds POST /command from the required name plus the
// convenience field (movie_ids), merged with any extra caller-supplied
// params.
...
func (radarrPlugin) Invoke(req plugin.InvokeRequest) (plugin.InvokeResult, error) {
	conn := req.Connection
	if conn == nil {
		conn = map[string]any{}
	}
	...
	if baseURL == "" {
		return plugin.InvokeResult{}, plugin.Errorf(plugin.CodeInvalidParams, "connection.base_url is required (e.g. http://radarr:7878)")
	}
...
func apiURL(rawBase string) (string, error) {
	...
	if u == "" {
		return "", fmt.Errorf("connection.base_url is required (e.g. http://radarr:7878)")
	}
	return u + "/api/v3", nil
}
...
// value; nil for an empty body (e.g. a 200/204 from delete_movie with
// nothing to report).
...
//
// Radarr's Webhook connection has no signing of its own — it just POSTs an
...
func requireToken(secret string, allowUnsigned bool) error {
	...
	if allowUnsigned {
		fmt.Fprintln(os.Stderr, "radarr: allow_unsigned is set — accepting UNAUTHENTICATED webhooks; anyone who can reach the listen address can fire triggers")
		return nil
	}
	return fmt.Errorf("radarr: no webhook.secret configured — an unauthenticated listener accepts any POST on the listen address as a real event. Set webhook.secret (and put it in Radarr's Webhook connection as an X-Conductor-Token header or ?token= query param), or set webhook.allow_unsigned: true if you genuinely front this with something else that authenticates")
}
...
// radarrQualityWrapper is the subset of Radarr's `release`/`movieFile`
// object this plugin reads for a quality name.
type radarrQualityWrapper struct {
	Quality *struct {
		Quality *struct {
...
// handleWebhookBody parses one Radarr Webhook delivery and emits a
...
func handleWebhookBody(body []byte, dedup *sourcekit.Dedup, emit func(any) error) {
	...
	if m, ok := payload["movie"].(map[string]any); ok {
	...
	dedupKey := dedupKey(eventType, movie.TmdbID, downloadID)
	if !dedup.Add(dedupKey) {
		return
	}

	title := fmt.Sprintf("radarr %s", eventType)
	if movie.Title != "" {
		title = fmt.Sprintf("radarr %s: %s", eventType, movie.Title)
	}
	...
	_ = emit(map[string]any{
		...
		"context": map[string]any{
			...
			// filter evaluator (filters: {event_types/movies: [...]}).
...
// extractQuality reads the release/movie-file quality name Radarr's Grab and
// Download webhooks carry, from whichever of `release` or `movieFile` is
// present.
func extractQuality(payload map[string]any) string {
	for _, key := range []string{"release", "movieFile"} {
...
// dedupKey builds the dedup key: eventType + tmdbId + (downloadId, when
...
func dedupKey(eventType string, tmdbID int64, downloadID string) string {
	disambiguator := downloadID
	if disambiguator == "" {
		disambiguator = strconv.FormatInt(time.Now().Unix(), 10)
	}
	return fmt.Sprintf("%s\x00%d\x00%s", eventType, tmdbID, disambiguator)
}

func main() {
	if err := plugin.Serve(radarrPlugin{}); err != nil {
		fmt.Fprintf(os.Stderr, "conductor-radarr: %v\n", err)
		os.Exit(1)
	}
}
